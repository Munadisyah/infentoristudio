...
// Ambil data pemilik sebelum proses update
$query = "SELECT * FROM user WHERE user_id='$user_id'";
$result = mysqli_query($koneksi, $query);
if (!$result) {
    echo '<script>alert("Gagal mengambil data: ' . mysqli_error($koneksi) . '"); window.location="pemilik_dashboard.php";</script>';
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $foto_profil = $pemilik['foto_profil'] ?? null;
            ...
            if (!is_writable($target_dir)) {
                ...
                if (!in_array($file_extension, $allowed_extensions)) {
                    ...
                    if (move_uploaded_file($_FILES["foto_profil"]["tmp_name"], $target_file)) {
                        ...
                        if ($pemilik['foto_profil'] && file_exists($pemilik['foto_profil'])) {
                            if (!unlink($pemilik['foto_profil'])) {
    ...
    if ($koneksi) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                echo '<script>alert("Profil berhasil diperbarui!"); window.location="pemilik_profil.php";</script>';
            } else {
...
// Ambil data pemilik untuk ditampilkan
$query = "SELECT * FROM user WHERE user_id='$user_id'";
$result = mysqli_query($koneksi, $query);
$pemilik = mysqli_fetch_assoc($result);
?>
...
<html lang="id">
<head>
    <title>Pemilik Profil - Infentori Studio</title>
    <meta charset="UTF-8">
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        <div class="sidebar-header">
            ...
            <h3><?php echo $pemilik['nama_lengkap']; ?></h3>
            <p>Pemilik</p>
        </div>
        ...
        <div class="profile-container">
            ...
            <form method="POST" class="profile-form" enctype="multipart/form-data">
                ...
                <?php if ($pemilik['foto_profil']): ?>
                    <img src="<?php echo $pemilik['foto_profil']; ?>" alt="Foto Profil">
                <?php endif; ?>
                <input type="file" id="foto_profil" name="foto_profil" accept="image/jpeg, image/png">

                <label for="nama_lengkap">Nama Lengkap</label>
                <input type="text" id="nama_lengkap" name="nama_lengkap" value="<?php echo $pemilik['nama_lengkap']; ?>" required>

                <label for="alamat">Alamat</label>
                <input type="text" id="alamat" name="alamat" value="<?php echo $pemilik['alamat']; ?>" required>

                <label for="email">Email</label>
                <input type="email" id="email" name="email" value="<?php echo $pemilik['email']; ?>" required>

                <label for="no_telp">No. Telepon</label>
                <input type="text" id="no_telp" name="no_telp" value="<?php echo $pemilik['no_telp']; ?>" required>
